...
from attention_firewall.focus_assist import get_focus_assist_info
...
class AttentionFirewallClient:
    ...
    async def _notification_forwarder(self) -> None:
        ...
        while self._running:
            try:
                ...
                try:
                    notif = await asyncio.wait_for(self.notification_queue.get(), timeout=1.0)
                except TimeoutError:
                    continue
    ...
    async def start(self) -> None:
        ...
        logger.info("Starting Attention Firewall client...")
        ...
        if status.get("status") != "healthy":
            ...
            logger.info(f"Server online - {sessions} sessions, {devices} devices")

        # Check Focus Assist state
        focus_info = get_focus_assist_info()
        logger.info(f"Focus Assist: {focus_info['description']}")
        if focus_info["recommendation"]:
            logger.info(f"  → {focus_info['recommendation']}")
